...
-- دالة clamp (لأنها غير موجودة في Lua العادية)
...
local function UpdateESP(Player, Drawings)
    ...
    local HealthColorIndex = clamp(3 - HealthPercent * 2, 1, 3)
...
-- دالة الحصول على الجزء المستهدف
...
local function TeleportBehindTarget()
    if not CurrentTarget or not CurrentTarget.Character then 
        -- إذا لا يوجد هدف، حاول العثور على هدف
        CurrentTarget = FindBestTarget()
        if not CurrentTarget then return end
    end
    
    local targetRoot = CurrentTarget.Character:FindFirstChild("HumanoidRootPart") or CurrentTarget.Character:FindFirstChild("Torso")
    ...
    -- رفع الموقع قليلاً ليكون على الأرض
    behindPosition = behindPosition + Vector3.new(0, 3, 0)
    ...
    if LocalPlayer.Character then
        ...
        if Humanoid then
            if Settings.Combo.Speed.Enabled then
                -- تطبيق السرعة الجديدة (16 هي السرعة الافتراضية)
